...
const FUNCTION_NAME = "github-repos";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!owner) {
      console.error(`[${FUNCTION_NAME}][${requestId}] Owner não fornecido`);
      return new Response(
        JSON.stringify({ error: "Owner parameter is required" }),
    ...
    if (!githubToken) {
      console.error(`[${FUNCTION_NAME}][${requestId}] Token não configurado`);
      return new Response(
    ...
    console.log(`[${FUNCTION_NAME}][${requestId}] Fetching repos for owner: ${owner}`);
    
    // Tenta primeiro como organização
    let response = await fetch(
      `https://api.github.com/orgs/${owner}/repos?sort=updated&per_page=100`,
      {
        headers: {
    ...
    // Se retornar 404, tenta como usuário
    ...
    if (!response.ok) {
      ...
      console.error(`[${FUNCTION_NAME}][${requestId}] GitHub API error:`, error);
      return new Response(
    ...
    console.log(`[${FUNCTION_NAME}][${requestId}] Returning ${data.length} repos`);
